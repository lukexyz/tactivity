...
class SmoothApp(App):
    ...
    async def on_load(self) -> None:
        """Bind keys here."""
        await self.bind("b", "toggle_sidebar", "Toggle Settings")
    ...
    async def on_mount(self) -> None:
        ...
        await self.view.dock(Pdog(), Placeholder(), edge="top")
        await self.view.dock(self.bar, edge="left", size=40, z=1)

        self.bar.layout_offset_x = -40


SmoothApp.run(log="textual.log", title="Tactivity (03_smooth_animation.py)")